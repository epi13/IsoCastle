...
- Added an official Godot 4.7.1 single-threaded WebAssembly target with reproducible export validation, localhost server, root-layout deployable ZIP, and browser-safe export filtering.
- Added Playwright 1.61.1 Chromium and Firefox smoke coverage for Web startup, keyboard/mouse input, audio activation, remapping, settings/save persistence, and literal inventory drag-and-drop.
- Added explicit Web filesystem synchronization after save/settings replacement, durable-storage diagnostics, user-initiated fullscreen handling, and corrupt-primary backup recovery coverage.
- Extended GitHub Actions to create and upload the Web ZIP and run the Chromium browser flow without enabling deployment.
...
- Added 626 deterministic assertions, interactive smoke automation, campaign playthrough automation, Web browser automation, and content/art/audio manifest validation.
